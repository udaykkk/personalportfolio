      ...
      <div className="relative group">
        ...
        >
          {images.map((image, index) => (
            <img 
              key={index}
              className={`rounded-md w-64 ${index === 0 ? 'ml-8' : 'ml-4'} flex-shrink-0 transition-all duration-300 hover:shadow-[8px_8px_10px_rgba(239,68,68,0.8)]`}
              src={image} 
              alt="placeholder" 
            />
          ))}
        ...
        <div className="absolute left-0 top-1/2 transform -translate-y-1/2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 -mt-2">
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="black" role="img" viewBox="0 0 24 24" width="24" height="24" aria-hidden="true" transform="rotate(180)">
